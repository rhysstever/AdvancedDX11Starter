...
float3 IndirectSpecular(TextureCube envMap, int   mips, Texture2D brdfLookUp, 
	SamplerState samp, float3 viewRefl, float NdotV, float roughness, float3 specColor)
{ 
...
// !!! VERY IMPORTANT !!!
// ---------------------- 
// You must update the signature for the DiffuseEnergyConserve() function that is already defined in Lighting.hlsli:
//  - Update the diffuse parameter to be a float3instead of a float
